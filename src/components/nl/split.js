// 增加了mouseenter和mouseleave来控制快捷按钮显隐
...
const NOOP = () => false;
const gutterMouseEnter = "_j"; // add by jin
const gutterMouseLeave = "_k"; // add by jin
...
const Split = (idsOption, options = {}) => {
  ...
  function mouseEnter(e) {
    ...
    getOption(options, "onMouseEnter", NOOP)(e);
  }

  // add by jin
  function mouseLeave(e) {
    getOption(options, "onMouseLeave", NOOP)(getSizes());
  }
  ...
  elements = ids.map((id, i) => {
    ...
    if (i > 0) {
      ...
      pair[gutterMouseEnter] = mouseEnter.bind(pair); // add by jin
      pair[gutterMouseLeave] = mouseLeave.bind(pair); // add by jin
      ...
      gutterElement[addEventListener]("mouseenter", pair[gutterMouseEnter]); // add by jin
      gutterElement[addEventListener]("mouseleave", pair[gutterMouseLeave]); // add by jin
  ...
  function destroy(preserveStyles, preserveGutter) {
    pairs.forEach((pair) => {
      if (preserveGutter !== true) {
        ...
        pair.gutter[removeEventListener]("mouseenter", pair[gutterMouseEnter]); // add by jin
        pair.gutter[removeEventListener]("mouseleave", pair[gutterMouseLeave]); // add by jin
      }
