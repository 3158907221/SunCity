...
示例：
  <nl-lazy-single-select v-model="qnteVoteTable.form.empId" :data="empList" filterable clearable placeholder="请选择" #default="sp" valueName="empId" :filterNames="['empNumber', 'empName']">
    <el-option v-for="item in sp.renderData" :key="item.empId" :label="item.empNumber + ' ' + item.empName" :value="item.empId"></el-option>
  </nl-lazy-single-select>
问题1：
...
-->
<template>
  <el-select v-bind="$attrs" v-on="$listeners" v-lazyload="lazyload" :filter-method="filter" @visible-change="handleVisibleChange">
  ...
  methods: {
    ...
    resetPage() {
      ...
      if (m > this.scrollContainHeight / 2) {
        this.scrollToTop(m - this.scrollContainHeight / 2 + 1, 100);
        // this.element.scrollTop = parseInt(m - this.scrollContainHeight / 2 + 1);
      } else {
        this.scrollToTop(1, 100);
    ...
    lazyload(element, direction) {
      if (direction > 0) {
        if (this.endPageIndex * this.pageSize < this.filterData.length) {
          let n = this.endPageIndex * this.pageSize;
          this.renderData = [...this.renderData, ...this.filterData.slice(n, n + this.pageSize)];
          this.endPageIndex++;
        }
      } else if (direction < 0) {
        if (this.beginPageIndex > 0) {
          this.beginPageIndex--;
          let n = this.beginPageIndex * this.pageSize;
          this.renderData = [...this.filterData.slice(n, n + this.pageSize), ...this.renderData];
          element.scrollTop = 1;
        }
      }
    },
