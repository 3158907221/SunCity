...
<script>
export default {
  name: "nl-lazy-filter-table-column",
  ...
  computed: {
    ...
    elTable() {
      ...
      while (parent && parent.$options.name !== "nl-lazy-filter-table") {
    ...
    // 字段是否已筛选
    hasFiltered() {
      return this.elTable.filterFuncList.findIndex((e) => e.type === this.type) > -1;
    },

    // 计算筛选选项
    filterAllOptions() {
      if (this.filterOptions.length > 0) return this.filterOptions;
      const parent = this.elTable;
      let arr = {};
  ...
  methods: {
    ...
    addStrFilter() {
      ...
      if (this.filterText.trim() === "") {
        this.filterSelect = "";
        this.elTable.runCommand("delFilter", this.type);
      ...
      this.elTable.runCommand("addFilter", { type: this.type, fun: temp });
      this.$refs.popover.doClose();
    },
    ...
    // 清空表格所有筛选
    clrFilter() {
      this.elTable.runCommand("clrFilter");
      this.$refs.popover.doClose();
    },
